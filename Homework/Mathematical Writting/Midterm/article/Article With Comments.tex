...
\usepackage[
    paperwidth=12in,    % Total width (A4 is 8.27in)
    paperheight=11.69in, % Standard A4 height
    left=1in,           % Sufficient left margin
    textwidth=6in,      % YOUR ORIGINAL TEXT WIDTH (Locked)
    marginparsep=0.5in, % Gap between text and notes
    marginparwidth=4in, % Huge 4-inch lane for reviewer notes
    top=1in, 
    bottom=1in
]{geometry}

%\usepackage[a3paper,margin=1in]{geometry}
%\usepackage[a4paper, left=0.5in, right=3.5in, top=1in, bottom=1in, marginparwidth=3in]{geometry}
...
\definecolor{colorLang}{RGB}{0, 80, 160}   % 类别1：语言标点（深蓝）
\definecolor{colorStyle}{RGB}{160, 80, 0}  % 类别2：风格冗余（棕橙）
\definecolor{colorMath}{RGB}{0, 120, 80}   % 类别3：数学排版（深绿）
\definecolor{colorLogic}{RGB}{180, 0, 0}   % 类别4：逻辑瑕疵（深红）
...
\DeclareDocumentCommand{\reviewer}{O{1} m m}{%
    \stepcounter{ReviewerNoteCount}%
    % 根据输入数字 1-4 选择颜色，如果不匹配则默认为黑色
    \colorlet{typeColor}{%
        \ifcase#1\or colorLang\or colorStyle\or colorMath\or colorLogic\else black\fi
    }%
    % 文中高亮
    {\color{typeColor}#2}%
    % 上标编号
    \textsuperscript{\color{typeColor}[\theReviewerNoteCount]}%
    % 侧边栏批注
    \marginnote{\linespread{1.1}\selectfont\color{typeColor}\scriptsize\raggedright\textbf{[\theReviewerNoteCount]} #3}%
}
\DeclareDocumentCommand{\reviewer}{O{1} O{0pt} m m}{%
    \stepcounter{ReviewerNoteCount}%
    \colorlet{reviewColor}{%
        \ifcase#1\or colorLang\or colorStyle\or colorMath\or colorLogic\else black\fi
    }%
    {\color{reviewColor}#3}% 文中高亮
    \textsuperscript{\color{reviewColor}[\theReviewerNoteCount]}% 编号
    % 使用 #2 作为垂直偏移参数，例如 [1.5ex] 或 [-10pt]
    \marginnote{\linespread{1.1}\selectfont\color{reviewColor}\scriptsize\raggedright\textbf{[\theReviewerNoteCount]} #4}[#2]%
}
% 语法: \begin{structuralreview}[类别数字]{侧边栏批注内容} ... \end{structuralreview}
% 默认类别为 4 (逻辑问题，深红色)
\NewDocumentEnvironment{structuralreview}{O{4} m}{%
    \stepcounter{ReviewerNoteCount}% 
    % --- 关键：锁定当前的序号 ---
    \edef\currentBoxNum{\theReviewerNoteCount}% 
    \colorlet{boxColor}{%
        \ifcase#1\or colorLang\or colorStyle\or colorMath\or colorLogic\else black\fi
    }%
    % 在边栏生成正确的序号和批注
    \marginnote{\linespread{1.1}\selectfont\color{boxColor}\scriptsize\raggedright\textbf{[\currentBoxNum]} #2}%
    ...
    \begin{tcolorbox}[
        ...
        % 使用锁定的 \currentBoxNum，确保右上角标号不随内部增加而改变
        overlay={
            \node[anchor=south east, color=boxColor, font=\scriptsize\bfseries] 
            at (frame.north east) {[\currentBoxNum]};
        }
    ]
}{%
    \end{tcolorbox}%
...
\begin{titlepage}
    ...
    \begin{flushleft}
        \large \textbf{1. Article Information} \\[0.5cm]
        \normalsize
        \begin{itemize}[leftmargin=2cm]
            \item \textbf{Title:} Projection, Measure, and Idempotent Relations: Independent Axioms and a Fixed-Point Coupling Law
            \item \textbf{Author:} Yunbeom Yi
            \item \textbf{Reviewer:} Nie Jiaduo, Wen Zijie and Zhong Haoyu
            \item \textbf{Link:} \href{arxiv.org/abs/2604.18640}{arXiv:2604.18640}
        \end{itemize}
    \end{flushleft}
    
    
    % --- Section 2: Error Category Legend ---
    \begin{flushleft}
        \large \textbf{2. Error Category} \\[0.5cm]
        \renewcommand{\arraystretch}{1.5}
        \begin{tabular}{|l|l|p{8.5cm}|}
            \hline
            \rowcolor{gray!10} \textbf{Marker Color} & \textbf{Category} & \textbf{Definition \& Description} \\ \hline
            {\color{colorLang} \textbf{Navy Blue}} & \textbf{Language \& Punctuation} &  \textbf{Violations of standard grammatical rules and typographical conventions.} This category encompasses basic syntactic deficiencies, redundant punctuation (e.g., errant commas), typos, and improper spacing within formulas. \\ \hline
            {\color{colorStyle} \textbf{Rust Orange}} & \textbf{Stylistic \& Structural} & \textbf{Impediments to narrative flow and structural clarity of academic expression.} This category addresses stylistic inefficiencies, including academic fluff, keyword overloading, excessive sentence nesting, and a lack of core thematic focus. \\ \hline
            {\color{colorMath} \textbf{Forest Green}} & \textbf{Mathematical Formatting} &  \textbf{Deviations from rigorous mathematical typesetting and exposition standards.} This category identifies missing punctuation in display equations, improper hyphenation of technical terms, and non-standard transitions within proofs.\\ \hline
            {\color{colorLogic} \textbf{Deep Red}} & \textbf{Axiomatic \& Logical Flaws} & \textbf{Fundamental breaches of mathematical reasoning and conceptual integrity.} Representing the most severe class of errors, this encompasses axiomatic redundancy, unstated cross-framework operations, and misleading or inaccurate terminology.\\ \hline
        \end{tabular}
    \end{flushleft}
\end{titlepage}
\newpage
...
\begin{abstract}
We introduce a minimal \reviewer[3]{ZFC--internal}{ZFC-internal} axiom system for \emph{pre-structural data}
\[
(X,\mathcal A,\mu,\mu^{\otimes2},R,I,\Pi_R,G,E_0,\eta),
\]
where $\Pi_R:X\to R$ is a designated map and $G\subseteq X\times X$ is a measurable relation. Those pre-structural data satisfying Axioms~I--III are called \emph{admissible structural models}; the axioms couple a finitely additive measure, an {idempotent retraction, and an idempotent symmetric} relation through a single coupling law (Axiom~III). The main results are the following. \reviewer{Consistency: the}{Consistency: The} axiom system is satisfiable in ZFC, with explicit finite and countable models, including finite families for which $\eta\neq 0$. 
\reviewer{Independence: the}{Independence: The} three axioms, and the three subclauses of Axiom~III (conservation, \reviewer[3]{projection--measure}{projection-measure} invariance, coupling law), are mutually independent. 
The coupling law admits a \reviewer[3][-2.5ex]{fixed--point}{fixed-point} reformulation: \reviewer{it is the unique}{It is the unique} bounded finitely additive solution of a \reviewer[3]{Banach--contraction}{Banach contraction (standard fixed term, no hyphen)} equation $f=T_\eta f$ determined by $(\mu,\Pi_R,\eta)$, with closed form $f_*(B)=\mu(B)+\tfrac{\eta}{1-\eta}\mu(\Pi_R^{-1}(B))$. Admissible structural models with fixed $\eta$ form a category $\mathbf{Struct}_\eta$; under fiber measurability and either a
 \reviewer[3][-2.5ex]{finiteness~(R--fin)}{finiteness (R-fin)} or countability plus 
 \reviewer[3][-2.5 ex]{$\sigma$--additivity}{$\sigma$-additivity}~(R--ctbl) hypothesis, a \reviewer[3]{quotient--factorization}{quotient-factorization} theorem reduces the classification problem to the \reviewer[3]{identity--retraction}{identity-retraction} case $\Pi_R=\mathrm{id}_X$, where each \reviewer[3]{$G$--equivalence}{$G$-equivalence} class $C_k$ satisfies $\mu(C_k)\in\{0,(1-\eta)^{-1}\}$ under the hypotheses of Theorem~\ref{thm:pi-id-classification}.
\end{abstract}

\bigskip

\noindent\textbf{Keywords:}
\begin{structuralreview}[2]{Lack of Focus}
...
\end{structuralreview}
% -------------------------------------------------------


\section{Introduction}

We present a small \reviewer[3]{ZFC--internal}{ZFC-internal} axiom system for abstract \emph{pre-structural data}, tuples
\[
\mathcal D=(X,\mathcal A,\mu,\mu^{\otimes2},R,I,\Pi_R,G,E_0,\eta)
\]
consisting of:
(i) a \reviewer[3]{set algebra}{algebra of sets (consistent with Definition 2.1)} $(X,\mathcal A)$ with a finitely additive measure $\mu:\mathcal A\to[0,\infty)$ and a finitely additive product charge $\mu^{\otimes2}$ on $\mathcal A\otimes\mathcal A$ satisfying the rectangle rule;
...
Our goal is to isolate a minimal \reviewer[3]{first--order}{first-order} framework in which projection, closure, and measure coexist and interact.
...
We use finite additivity to keep the axiom system minimal; Section~\ref{sec:sigma-additive} records the additional constraints under \reviewer[3]{$\sigma$--additivity}{$\sigma$-additivity}.

\paragraph{On terminology.}
We adopt the name \emph{curvature} for the idempotent symmetric relation $G$ and for the derived \reviewer[3]{two--point}{two-point} quantity $\mu^{\otimes2}((B\times X)\cap G)$ as a convenient label for a bilinear, \reviewer[3]{self--composing}{self-composing} interaction pattern. This usage is analogical rather than \reviewer[3]{differential--geometric}{differential-geometric}: no Riemannian, sectional, or Ricci curvature is defined or used in this paper. The terminology emphasizes that $G\circ G=G$ imposes a \reviewer[3]{closure--type}{closure-type} rigidity on pair interactions, in the same way that curvature operators in geometry control how infinitesimal data compose. The reader for whom this analogy is unhelpful may replace ``curvature'' with ``idempotent interaction'' throughout without loss of content.
...
\begin{itemize}[nosep]
  \item $G\circ G = G$ encodes an idempotent closure/\reviewer[3]{equivalence--type}{equivalence-type} behavior at the level of relations.
...
\begin{itemize}[nosep]
  \item \textbf{Measure/Probability}: if one normalizes $\mu(X)=1$, the measure component can be 
  \reviewer[1]{read}{regarded} as a finitely additive probability.
  \item \textbf{Closure/Topology}: $G$ induces a closure/connectedness pattern via its idempotence.
  \item \textbf{Equivalence}: on each \reviewer[3]{$G$--connected}{$G$-connected} component, idempotence forces transitivity along \reviewer[3]{$G$--chains}{$G$-chains}.
  \item \textbf{Projection}: $\Pi_R$ preserves $\mu$ on measurable subsets of $R$ (Axiom~III).
\end{itemize}
Thus, measure, projection, and closure appear as compatible components of a single \reviewer[3]{ZFC--internal}{ZFC-internal} structure.

\subsection{Contributions}\label{sec:contributions}
The contributions can be summarized as follows.
\begin{itemize}[nosep]
  \item A \reviewer[3]{ZFC--internal}{ZFC-internal} axiom system (Section~\ref{sec:axioms}) and explicit model families establishing consistency (Section~\ref{sec:models}), including finite families with $\eta\neq 0$.
  ...
  \item A \reviewer[3][-2ex]{fixed--point}{fixed-point} reformulation of the coupling law as the \reviewer{Banach--contraction}{Banach contraction} equation $f=T_\eta f$, with closed form $f_*(B)=\mu(B)+\tfrac{\eta}{1-\eta}\mu(\Pi_R^{-1}(B))$ and a \reviewer[3]{Neumann--series}{Neumann series} expansion (Theorem~\ref{thm:coupling-fixed-point}, Corollary~\ref{cor:coupling-identification}).
  \item A \reviewer[3]{quotient--factorization}{quotient-factorization} theorem reducing admissibility to the \reviewer[3][2ex]{identity--retraction}{identity-retraction} case $\Pi_R=\mathrm{id}_X$ under fiber measurability and either (R--fin) or (R--ctbl) (Theorem~\ref{thm:quotient-factorization}). In that case, under the hypotheses of Theorem~\ref{thm:pi-id-classification}, each \reviewer[3]{$G$--equivalence}{$G$-equivalence} class carries mass $0$ or $(1-\eta)^{-1}$.
\end{itemize}
Section~\ref{sec:sigma-additive} records a side observation: under \reviewer[3]{$\sigma$--additivity}{$\sigma$-additivity} with $\mu(X)=1$, the global instance of the coupling law forces $\eta=0$; so nontrivial $\eta\neq 0$ families appear either in the finitely additive setting or for \reviewer[3]{$\sigma$--additive}{$\sigma$-additive} measures not normalized as probabilities.


\section{Language and basic objects}

We recall standard notions used throughout the paper.
\begin{structuralreview}[2]{This definition completely impenetrable to any reader lacking prior background knowledge.}
...
(Under \reviewer[3][-2ex]{$\sigma$--additivity}{$\sigma$-additivity} and \reviewer[3]{$\sigma$--finiteness}{$\sigma$-finiteness}, the standard product charge exists uniquely; see Section~\ref{sec:sigma-additive}.)
\end{definition}

\begin{lemma}[Atomic product charge exists]\label{lem:atomic-product-charge}
Assume $X$ is \reviewer{finite or countable}{finite(resp. countable)} \reviewer[1][2ex]{and}{, and} $\mathcal A=\mathcal P(X)$. Suppose $\mu$ is atomic, i.e.\ there exists
...
In particular, in the \reviewer{finite/countable}{finite(resp. countable)} atomic regimes used in Section~5, a product charge can be taken canonically.
\end{lemma}
...
\begin{structuralreview}[2]{Theorem 3's clause (c) is cited more than once in the following text, but it is not numbered here.}
$\mu(R)+\mu(I)=E_0>0$.
...
\begin{proof}
\reviewer[1][-2ex]{(i) follows from}{Proposition (i) follows from} $\Pi_R(X)\subseteq R$ and $\Pi_R|_R=\mathrm{id}_R$ (Axiom~I). \reviewer{(ii) is a consequence}{Proposition (ii) is a consequence} of the idempotence $\Pi_R\circ\Pi_R=\Pi_R$: for $n=1$ this is definitional, and for $n\ge 2$ we have $\Pi_R^n(x)=\Pi_R(\Pi_R^{n-1}(x))=\Pi_R(x)$ by induction. \reviewer{(iii) is immediate}{Proposition (iii) is immediate} from Axiom~II as stated.
\end{proof}
...
while being forced to carry no \reviewer[3]{$\mu$--mass}{$\mu$-mass} under full-partition invariance.
...
We record an \reviewer[1][-2ex]{operator--theoretic}{operator-theoretic} reformulation of \reviewer{subclause~(c) of Axiom~III}{subclause (c) of Axiom III} (the coupling law) as a \reviewer[3]{fixed--point}{fixed-point} equation determining a unique bounded extension of $\mu$ under the projection $\Pi_R$.
...
\begin{itemize}[nosep]
  ...
  \item \emph{\reviewer[3]{Self--similar}{Self-similar} propagation along $\Pi_R$}: the correction to $\mu$ propagates under the preimage map $\Pi_R^{-1}$, scaled by $\eta$.
\end{itemize}
...
Formally iterating $T_\eta$ from any $f_0\in\mathcal B$ and using Banach convergence, one obtains the \reviewer[3]{Neumann--series}{Neumann series} expression
\[
...
When $\Pi_R=\mathrm{id}_X$, one has $\Pi_R^{-1}(B)=B$, so the \reviewer[3]{fixed--point}{fixed-point} formula collapses to
\[
f_*(B)=\frac{\mu(B)}{1-\eta}\qquad(B\ \text{measurable}).
\]
In regimes where the full hypotheses of Theorem~\ref{thm:pi-id-classification} hold for the relevant \reviewer[3]{identity--retraction}{identity-retraction} core, this yields the blockwise normalization law: every \reviewer[3]{positive--mass}{positive-mass} \reviewer[3][2.5ex]{$G$--equivalence}{$G$-equivalence} class $C_k$ satisfies $\mu(C_k)=(1-\eta)^{-1}$.
\end{remark}
...
In particular, $(B\times X)\cap G$ and $(B\times R)\cap(G\cap(R\times R))$ differ by a \reviewer[3]{$\mu^{\otimes2}$--null}{$\mu^{\otimes2}$-null} set, yielding the stated equality.
...
\]
By the decoupling hypothesis, the second term has \reviewer[3]{$\mu^{\otimes2}$--measure}{$\mu^{\otimes2}$-measure} zero, hence
\[
...
\begin{proof}
Let $X$ be finite and nonempty\reviewer[1][-3ex]{, and}{The comma should be deleted.} choose disjoint nonempty subsets 
\reviewer{$R,I\subseteq X$}{There should be a period.}
Let $X$ be finite and nonempty, and choose disjoint nonempty subsets $R,I\subseteq X$
\reviewer{with $R\cup I = X$.}{I barely know what goes wrong to say this again with an extra condition.}
...
Since $\mathcal A$ is the full power set, \reviewer{$\Pi_R^{-1}(B)\in\mathcal A$}{There should be a subject preceding this to avoid concatenation with the comma.} for all $B\subseteq R$,
so Axiom~I holds.
...
\]
Then $\mu$ is finitely additive\reviewer{, $\mu(I)=0$, and $\mu(R)>0$.}{Suggested modification: ... and satisfies $\mu(I)=0$ and $\mu(R)>0.$}
Set
\[
...
\reviewer{For}{Wrong capitalizing.} any $B\subseteq X$,
\[
(B\times X)\cap G
= \{(x,x): x\in B\},
\]
\reviewer{and}{The preceding comma should be deleted.} therefore
\[
\mu^{\otimes2}((B\times X)\cap G)
= \sum_{x\in B} \mu(\{x\})^2
= \sum_{x\in B} m(x)^2
...
Then $\mathcal M=(X,\allowbreak \mathcal P(X),\allowbreak \mu,\allowbreak \mu^{\otimes2},\allowbreak R,\allowbreak I,\allowbreak \Pi_R,\allowbreak G,\allowbreak E_0,\allowbreak \eta)$ satisfies Axioms I--III\reviewer{, with}{Delete the comma.}
...
If $B\subseteq R$, then $\Pi_R^{-1}(B)$ equals $B$ or $B\cup\{i\}$ depending on whether $r_0\in B$\reviewer{, and}{. And} $m(i)=0$ yields
$\mu(\Pi_R^{-1}(B))=\mu(B)$.
...
\reviewer[1][-2ex]{Thus the coupling law reduces to checking}{Thus the coupling law is proved if we check...} for each \reviewer{$r\in R$:}{$r\in R$ the following proposition: }
\[
m(r)^2=m(r)+\eta\,m(r)^2 \iff (1-\eta)m(r)^2=m(r),
\]
\reviewer{which}{Change the comma into a period and change ``which'' into ``This''.} holds for $m(r)=\frac{1}{1-\eta}$.
\end{proof}

\begin{proposition}[A finite model with $\eta\neq 0$ and non-diagonal curvature]\label{prop:eta-nonzero-nondiag}
\reviewer{Let}{There seems to be a syntax adding a period after the parenthesis.} $X=\{r_0,r_1\}$ and take $R=X$, $I=\varnothing$, $\Pi_R=\mathrm{id}_X$.
Fix any $\eta\in[0,1)$\reviewer[2]{, $\eta\neq 0$.}{Superfluous.} Define $\mu$ by
\[
m(r_0)=m(r_1)=\frac{1}{2(1-\eta)},\qquad \mu(B)=\sum_{x\in B}m(x),
\]
\reviewer{so}{The preceding formulae should be divided by ``and'' and delete the comma preceding ``so''.} that $\mu(X)=\frac{1}{1-\eta}$. Let $G=X\times X$ (the total relation on $X$).
...
Then $\mathcal M=(X,\allowbreak \mathcal P(X),\allowbreak \mu,\allowbreak \mu^{\otimes2},\allowbreak R,\allowbreak I,\allowbreak \Pi_R,\allowbreak G,\allowbreak E_0,\allowbreak \eta)$ satisfies Axioms I--III with $E_0=\mu(R)=\frac{1}{1-\eta}$\reviewer{, and}{Delete the comma.} $G$ is non-diagonal.
\end{proposition}

\begin{proof}
Invariance is immediate since $\Pi_R=\mathrm{id}_X$. \reviewer{For any $B\subseteq X$, $(B\times X)\cap G=B\times X$}{Add ``we have'' before this formula to avoid touching the comma.}, hence
\[
...
Then $G$ is an equivalence relation on $X$; denote its equivalence classes by $(C_k)_{k\in K}$\reviewer{, so}{Delete the comma.} that
\[
G=\bigcup_{k\in K}(C_k\times C_k)\qquad\text{(disjoint union of blocks)}.
\]
Let $\mu$ be a finitely additive measure on $\mathcal P(X)$ and let $\mu^{\otimes2}$ be the product charge on the product algebra $\mathcal P(X)\otimes\mathcal P(X)$, satisfying the rectangle rule of Definition~\ref{def:product-measure}. Suppose further that one of the following summability hypotheses holds\reviewer{, so}{Delete the comma.} that the block formula below is well--defined:
\begin{itemize}[nosep]
\item[\emph{(K--fin)}] \reviewer[1][-2ex]{the}{The} index set $K$ is finite\reviewer{; or}{Delete the semicolon.}
\item[\emph{(K--ctbl)}] \reviewer{$K$}{There should be a subject preceding this and the same modification for the next $\mu$} is countable, $\mu$ is $\sigma$--additive (so $\mathcal P(X)$ is treated as a $\sigma$--algebra) and $\sigma$--finite on $X$, and $\mu^{\otimes2}$ is the standard $\sigma$--additive product measure on the product $\sigma$--algebra $\mathcal P(X)\otimes\mathcal P(X)$ (\reviewer{which}{Just put this out of the parenthesis.} then exists and is uniquely characterized by the rectangle rule).
\end{itemize}
...
\]
\reviewer{holds}{The ``for all'' symbol is inappropriate and the following formula should be preceded by ``we have''.} if and only if, for every class $C_k$ with $\mu(C_k)>0$,
\[
\mu(C_k)=\frac{1}{1-\eta}.
\]
...
\]
\reviewer{and}{Delete the comma.} the coupling law is precisely the blockwise normalization law above.
\end{theorem}

\begin{proof}
First, \reviewer{$G$}{Touching the comma.} is transitive: if $(x,y)\in G$ and $(y,z)\in G$, then $(x,z)\in G\circ G=G$.
...
\]
\reviewer{a disjoint union}{Inappropriate commas and no verbs.}. \reviewer{Under~(K--fin) finite additivity of $\mu^{\otimes2}$ together with the rectangle rule gives}{There is no subject.}
\[
\mu^{\otimes2}((B\times X)\cap G)=\sum_{k\in K}\mu(B\cap C_k)\,\mu(C_k)
\]
immediately. Under~(K--ctbl) the same identity follows from $\sigma$--additivity of $\mu^{\otimes2}$ applied to the countable disjoint union \reviewer{together with the rectangle rule.}{If I am right, this is part of the argument rather than a adjective for ``union'' so there is an error in the sentence arrangement.} In either case we obtain the stated block formula.

Since $\Pi_R=\mathrm{id}_X$, the coupling law \reviewer[3]{reduces}{Again the logician's symbols are not proper.} to
\[
...
\begin{proof}
By Theorem~\ref{thm:pi-id-classification}, \reviewer{$\mu(C_j)=\frac{1}{1-\eta}$ for each $j$.}{Avoid touching the comma.}
...
\]
In particular, at fixed $\eta\neq 0$ the coupling law imposes exactly $m$ scalar constraints (one per $G$-component)\reviewer{, and}{Delete the comma.} imposes no further constraint on the distribution of $\mu$ inside each block.
\end{theorem}

\begin{proof}
For \reviewer{any $B\subseteq X$,}{The formula touching the comma.}
\[
...
\]
Since $\Pi_R^{-1}(B)=B$, the coupling law is equivalent \reviewer{to}{Logic symbols should be changed into words.}
\[
...
Let $X$ be a finite nonempty set\reviewer{, partitioned}{Delete the comma.} into nonempty equivalence classes
$(C_j)_{j\in J}$.
...
\]
Define $\Pi_R:X\to R$ by $\Pi_R(x)=r_j$ whenever $x\in C_j$\reviewer{, and define}{and}
\[
...
\begin{proof}
By \reviewer{construction}{Delete the comma.}, $R,I\in\mathcal A$, $R\cap I=\varnothing$, and $R\cup I=X$.
The map $\Pi_R$ restricts to the identity on $R$\reviewer{, and}{Delete the comma.} $\Pi_R\circ\Pi_R=\Pi_R$ because
...
\]
\reviewer{and}{Delete the comma.} therefore
\[
\mu(\Pi_R^{-1}(B))
...
\]
\reviewer{disjoint union}{No verb.}.
Hence
\[
\mu^{\otimes2}((B\times X)\cap G)
...
= \mu(B),
\]
using $\mu(B_j)=w_j$ if $r_j\in B_j$ and $0$ otherwise\reviewer{, and}{Delete the comma.} $w_j^2=w_j$ for $w_j\in\{0,1\}$.
...
be a finite structural model as in Theorem~\ref{thm:finite-model}\reviewer{, with $\eta_0=0$.}{Delete the comma.}
...
More concretely\reviewer{, $\mathcal A$}{Touching the comma.} consists of all sets of the form
\[
...
\]
\reviewer{so}{Delete the comma.} that all points of $Y$ have measure zero.
Set
\[
R := R_0,\qquad
I := I_0 \sqcup Y.
...
\reviewer{and}{The sentence should be divided here into two.} their union is block--diagonal with respect to the decomposition $X=X_0\sqcup Y$.
\reviewer{Moreover,}{Delete the comma.} $G\in\mathcal A\otimes\mathcal A$.
...
Equip $X\times X$ with the product charge $\mu^{\otimes2}$.
If $B\in\mathcal A$ and we write $B=B_0\sqcup B_Y$ \reviewer{with}{... and...}
...
\reviewer{Similarly,}{Touching the comma.}
\[
\mu(B) = \mu_0(B_0).
\]
...
\reviewer{Thus}{The beginning of a line should not be a formula.} $\mathcal M=(X,\mathcal A,\mu,\mu^{\otimes2},R,I,\Pi_R,G,E_0,\eta)$ is a countably infinite structural model.
\end{proof}
...
\reviewer[1][-3ex]{In}{I suspect whether the author is fully aware of the rules adopted for indentation.} this section we establish that the three axioms of Section~\ref{sec:axioms} are mutually independent: \reviewer{no}{Upper case.} axiom follows from the remaining two. Together with the model constructions of Section~\ref{sec:models}, \reviewer{which}{This clause should be replaced by a simple sentence to decrease the length of the sentence.} establish consistency, this shows that the axiom system is both consistent and minimal, justifying the word ``minimal'' used in the abstract and Section~\ref{sec:contributions}.

Independence is established in the model-theoretic sense: \reviewer{for}{For} each axiom $\mathsf X\in\{\mathrm{I},\mathrm{II},\mathrm{III}\}$ we exhibit a structure over the same signature which satisfies the other two axioms but fails $\mathsf X$. We also record the stronger result that the three subclauses of Axiom~III (conservation, projection--measure invariance, coupling law) are themselves mutually independent within Axiom~III.

\subsection{Separating models for Axioms I, II, III}

The constructions below follow a minimal relaxation principle: \reviewer{each}{Each} separating datum $\mathcal D_{\neg \mathsf X}$ is obtained from a baseline finite model (as in Section~\ref{sec:models}) by modifying exactly the feature asserted by the axiom under analysis, leaving the remaining structural features intact.
...
\begin{structuralreview}[2]{This proof should be seperated into several sentences using a single adverbial clauses instead of exploiting parentheses over and over again especially when parentheses would appear inside of parentheses.}
...
Define $\mu:\mathcal A\to[0,\infty)$ by the point weights $m(a)=m(b):=1$ and $m(c):=0$\reviewer{, and}{Delete the comma.} let $\mu^{\otimes2}$ be the canonical atomic product charge from Lemma~\ref{lem:atomic-product-charge}. Set $\eta:=0$ and $E_0:=\mu(R)+\mu(I)=2>0$, so Axiom~III(a) (conservation) holds.
...
\]
Then $G$ is reflexive ($\Delta_X\subseteq G$) and symmetric. \reviewer[1][-3ex]{However}{Touching the comma.}, $G$ is not idempotent: \reviewer{the}{The} pair $(a,c)$ is obtained via
\[
(a,b)\in G,\quad (b,c)\in G \;\Longrightarrow\; (a,c)\in G\circ G,
\]
\reviewer{but}{Delete the comma.} $(a,c)\notin G$, so $G\circ G\supsetneq G$. Hence Axiom~II fails.
...
\end{align*}
\reviewer{and}{Delete the comma.} finite additivity extends this to all $B\subseteq X$. With $\eta=0$ and $\Pi_R=\mathrm{id}_X$, the coupling law reduces to $\mu^{\otimes2}((B\times X)\cap G)=\mu(B)$, which we have verified. Hence Axiom~III(c) holds. All of Axiom~III is therefore satisfied, while Axiom~II fails.
\end{proof}

\begin{remark}[On the $\mu^{\otimes2}$-invisibility of the failure of Axiom~II]\label{rem:II-invisibility}
In the datum $\mathcal D_{\neg \mathrm{II}}$ above, the failure of $G\circ G=G$ occurs at the pair $(a,c)$, \reviewer{which carries}{The formula occurs at the beginning of a line.} positive $\mu^{\otimes2}$-weight ($m(a)m(c)=1$) but lies outside $G$. The separating witness is set-theoretic ($G\circ G\supsetneq G$ as subsets of $X\times X$), not measure-theoretic. This reflects the fact that Axiom~II is a structural rather than measure-theoretic statement; the failure is visible at the level of the relation $G$ itself, regardless of whether the measure detects it through the coupling law.
\end{remark}
...
\begin{proof}
Let $X=\{r,i\}$ and $\mathcal A=\mathcal P(X)$. Set $R:=\{r\}$, $I:=\{i\}$\reviewer{, and}{Delete the comma.} define $\Pi_R:X\to R$ by $\Pi_R(r):=r$, \reviewer{$\Pi_R(i):=r$.}{Insert ``and'' before instead of a comma.} Then $\Pi_R$ is idempotent\reviewer[1][2ex]{, $\Pi_R|_R=\mathrm{id}_R$}{Formula touching the comma.}, and preimages of measurable subsets of $R$ are measurable. Hence Axiom~I holds.

Set $G:=\Delta_X$, satisfying Axiom~II.

Define $\mu$ by $m(r):=m(i):=1$\reviewer{, and}{Delete the comma.} let $\mu^{\otimes2}$ be the atomic product charge. Set $\eta:=0$ and $E_0:=\mu(R)+\mu(I)=2>0$, so Axiom~III(a) (conservation) holds.

However, Axiom~III(b) (projection--measure invariance) fails: \reviewer{taking}{Taking} $B=\{r\}\subseteq R$,
\[
\Pi_R^{-1}(\{r\})=X=\{r,i\},
\qquad
\mu(\Pi_R^{-1}(\{r\}))=m(r)+m(i)=2,
\]
while $\mu(\{r\})=1$. Hence $\mu(\Pi_R^{-1}(B))\ne\mu(B)$\reviewer{, and}{Delete the comma.} Axiom~III(b) is violated. Consequently Axiom~III as a whole fails.
\end{proof}
...
The datum $\mathcal D_{\neg \mathrm{III}}$ of Proposition~\ref{prop:indep-III} plays a second role beyond separating Axiom~III: \reviewer{it}{It} witnesses the necessity of the Axiom~III(b) hypothesis in part~(iii) of Theorem~\ref{thm:quotient-factorization}. \reviewer{Explicitly, $\mathcal D_{\neg \mathrm{III}}$}{Touching the comma.} satisfies Axioms~I, II, III(a), III(c), fiber measurability, the regularity hypothesis (R--fin), and the full partition $X=R\sqcup I$; yet it is not admissible (because III(b) fails), while its restriction $\mathcal D_{\neg\mathrm{III}}|_R$ is the one--point identity--retraction datum $(\{r\},\{r\})$ with total mass $1$ and trivial relation $\Delta_{\{r\}}$, which is admissible. Hence without the Axiom~III(b) hypothesis, admissibility of $\mathcal D|_R$ would not imply admissibility of $\mathcal D$.
\end{remark}

\subsection{Internal independence of Axiom III}\label{sec:indep-III-internal}

Axiom~III comprises three distinct \reviewer{assertions}{The nouns below should be capitalized.}:
\begin{itemize}[nosep]
...
We show that these three subclauses are independent: \reviewer{no}{No} one of them follows from Axioms~I, II together with the other two.

\begin{proposition}[Internal independence of Axiom III]\label{prop:indep-III-internal}
Subclauses {\rm(a)}, {\rm(b)}, {\rm(c)} of Axiom~III are mutually independent\reviewer{, relative}{Delete the comma.} to Axioms~I and~II.
\end{proposition}

\begin{proof}
We exhibit three structures satisfying Axioms~I,~II and exactly two of {\rm(a), (b), (c)}.

\emph{(a) is independent.} Let $X=\{r\}$, $R=\{r\}$, $I=\varnothing$, $\Pi_R=\mathrm{id}_X$, $G=\Delta_X$, and $\mu\equiv 0$ (the zero charge)\reviewer{, with}{Delete the comma.} $\eta=0$. Axioms~I and~II hold. Subclause~(b) is trivial ($\Pi_R=\mathrm{id}$), and subclause~(c) holds (both sides equal $0$). However\reviewer{, $\mu(R)+\mu(I)=0$}{we have...}, so no $E_0>0$ satisfies conservation\reviewer{; subclause~(a) fails.}{This should be a result instead of a contrast.} This shows (a) is not derivable from Axioms~I, II, (b), (c).

\emph{(b) is independent.} The structure $\mathcal D_{\neg \mathrm{III}}$ of Proposition~\ref{prop:indep-III} satisfies Axioms~I,~II and subclause~(a) (with $E_0=2$)\reviewer{, and}{Delete the comma.} a direct check (as in the proof of Proposition~\ref{prop:indep-III}) shows that the coupling law~(c) holds with $\eta=0$: \reviewer{for}{For} $B=\{r\}$, $\mu^{\otimes2}((\{r\}\times X)\cap \Delta_X)=m(r)^2=1=\mu(\{r\})$\reviewer{, and}{Delete the comma.} similarly for $B=\{i\}$ and $B=X$. However, subclause~(b) fails as shown in Proposition~\ref{prop:indep-III}. Hence (b) is not derivable from Axioms~I,~II,~(a),~(c).

\emph{(c) is independent.} \reviewer{Let}{Superfluous commas.} $X=\{a,b\}$, $R=X$, $I=\varnothing$, $\Pi_R=\mathrm{id}_X$, $G=X\times X$, with point weights $m(a)=1$, $m(b)=0$, and $\eta=1/2$. Axioms~I and~II hold (the total relation $X\times X$ is reflexive, symmetric, and idempotent). Subclause~(a): $\mu(R)+\mu(I)=1>0$, \reviewer{take $E_0=1$.}{The comma should be replaced by ``and''.} Subclause~(b): $\Pi_R=\mathrm{id}_X$ makes invariance trivial. However, for $B=\{a\}$,
\[
...
\item Axiom~II governs the \emph{closure structure} of the interaction relation. Its failure \reviewer{in}{The beginning of a line is a formula.} $\mathcal D_{\neg\mathrm{II}}$ is set-theoretic ($G\circ G\supsetneq G$), independent of measure.
\item Axiom~III governs the \emph{quantitative coupling} between measure, projection, and relation. Its three subclauses separate cleanly: \reviewer{conservation}{Capitalize.} fixes total mass, invariance governs pointwise behavior under $\Pi_R$, and the coupling law governs the two-point curvature load.
\end{itemize}
\end{remark}
...
\]
\reviewer{together}{The things in the parenthesis should be replaced by words without any parenthesis and the comma should be deleted.} with $\sigma$--additivity on $\Sigma\otimes\Sigma$.

We emphasize that the conclusions below use only the $B=X$ instance of the coupling law,
\reviewer{and}{Delete the comma.} thus apply in any $\sigma$--additive setting where $\mu^{\otimes2}$ is defined.
...
\begin{proof}
By Proposition~\ref{prop:global-constraint}\reviewer{, $\mu^{\otimes2}(G)=M/(1-\eta)$.}{Touching the comma.}
...
\begin{proof}
From Proposition~\ref{prop:global-constraint}\reviewer{, $\mu^{\otimes2}(G)=1/(1-\eta)\ge 1$.}{Touching the comma.}
...
\]
\reviewer{and}{Delete the comma.} moreover $\mu^{\otimes2}(G)=\frac{M}{1-\eta}\in(0,M^2]$.
In particular\reviewer[1][-3ex]{, $\eta\neq 0$}{Touching the comma.} is possible only when $M>1$ (i.e.\ the measure is not probability-normalized).
\end{proposition}
...
\sloppy
In an approximating sequence of finite models $(X_n,\allowbreak \mu_n,\allowbreak G_n,\allowbreak \Pi_{R,n},\allowbreak \eta_n)$, the limiting behavior of $\eta_n$ is fundamentally normalization--dependent. Under standard probability normalization ($\mu_n(X_n)=1$), the global constraint typically forces $\eta_n\to 0$, often rendering the coupling trivial. Conversely, in regimes where $\mu_n(X_n)=M_n\to M>1$\reviewer{, $\eta_n$}{Touching the comma.} can stabilize to a nonzero constant compatible with the total mass $M$. In large--mass regimes ($M_n\to\infty$), the global identity does not by itself force $\eta_n\to 0$, shifting the structural burden to local instances of the coupling law (varying $B$), which become essential for characterizing the infinitesimal structure of the limit.
\end{remark}
...
either (i) in the finitely additive setting\reviewer{, or}{Delete the comma.} (ii) for $\sigma$--additive measures that are not normalized as probabilities.
The global constraint arises solely from the $B=X$ instance of the coupling law; under a fixed normalization $\mu(X)=M$, the admissible range of $\eta$ shrinks as $M$ decreases\reviewer{, and}{, so} any sequence of models in which $\eta$ is to stabilize away from zero therefore requires $M$ to grow accordingly.
\end{remark}
...
We \reviewer[1]{record}{introduce} a basic \reviewer[2][2ex]{categorical organization}{categorical structure} of structural models: the appropriate notion of \reviewer[1][1ex]{morphism}{morphisms}, the resulting category\reviewer[1][3ex]{, and}{delete this comma} the preservation of the coupling law under morphisms.

\subsection{\texorpdfstring{Morphisms and the category $\mathbf{Struct}$}{Morphisms and the category Struct}}

Throughout this section we fix the scalar $\eta\in[0,1)$, \reviewer[1]{but}{Replace 'but' with 'while' or 'whereas' for a smoother and more formal logical transition} treat $E_0\in(0,\infty)$ as \reviewer[1][2ex]{\emph{object data}}{an object datum}: 
it is recorded as part of each structural model and is allowed to vary between objects. 
Morphisms interact with $E_0$ via~(M4) below, which yields an automatic inequality $E_0'\le E_0$ (see Remark~\ref{rem:E0-morphism}), rather than \reviewer[1]{an a}{a} priori identification. From this point on, $\mathbf{Struct}_{\eta}$ denotes the category whose objects are admissible structural models (Definition~\ref{def:admissible}) with \reviewer[1][-2ex]{common $\eta\in[0,1)$}{a common value of $\eta\in[0,1)$}, and whose morphisms are defined below.
\begin{structuralreview}[2]{Typesetting Issue: The layout is overly congested. Inline 10-tuples distort line spacing. Recommend using display math for object signatures and increasing vertical separation in the (M1)-(M4) list to improve scannability.}
\begin{definition}[Morphism of structural models]\label{def:morphism}
Let $\mathcal M=(X,\allowbreak\mathcal A,\allowbreak\mu,\allowbreak\mu^{\otimes2},\allowbreak R,\allowbreak I,\allowbreak\Pi_R,\allowbreak G,\allowbreak E_0,\allowbreak\eta)$ and \reviewer[3]{$\mathcal M'=(X',\allowbreak\mathcal A',\allowbreak\mu',\allowbreak\mu'^{\otimes2},\allowbreak R',\allowbreak I',\allowbreak\Pi_{R'},\allowbreak G',\allowbreak E_0',\allowbreak\eta)$}{$(\mu')^{\otimes 2}$} be admissible structural models (Definition~\ref{def:admissible}) sharing a common value of $\eta\in[0,1)$; 
\reviewer[2]{the scalars $E_0,E_0'$ are part of the object data and need not agree a priori}{Definitions should be purely structural. Move this clarification to a Remark.}. A \emph{morphism} $\phi:\mathcal M\to\mathcal M'$ is a map $\phi:X\to X'$ satisfying:
...
\item \reviewer[1]{\emph{Measure--preservation}}{Measure preservation}: $\mu(\phi^{-1}(B'))=\mu'(B')$ for every $B'\in\mathcal A'$, and \reviewer[3]{$\mu^{\otimes2}((\phi\times\phi)^{-1}(S'))=\mu'^{\otimes2}(S')$}{$(\mu')^{\otimes 2}$} for every $S'\in\mathcal A'\otimes\mathcal A'$.
\end{enumerate}
\end{definition}
\end{structuralreview}
...
\]
If $\mathcal M$ has \reviewer[1]{full partition}{a full partition} ($X=R\sqcup I$), then $\mu(X)=E_0$ and hence $E_0'\le E_0$. Equality $E_0'=E_0$ holds when, in addition, $\phi^{-1}(R'\cup I')=X$, for instance when $\phi$ is surjective and $\mathcal M'$ also has \reviewer[1]{full partition}{a full partition}. Morphisms are thus \reviewer[1][-2ex]{in general}{Ambiguous quantifier}
\reviewer[1]{mass--non--increasing, and mass--preserving in the natural full--partition surjective case.}{mass-non-increasing,mass-preserving,full-partition} No separate \reviewer[1][1.5ex]{scalar--identification}{scalar identification} axiom is imposed, since the relation between $E_0$ and $E_0'$ is already controlled by~(M4).
\end{remark}
...
\item For $B'\subseteq R'$ measurable, $\mu'(B')=\mu(\phi^{-1}(B'))$, which combined with projection--measure invariance in $\mathcal M'$ recovers invariance in $\mathcal M$ \reviewer[1]{along $\phi$}{under the map $\phi$}.
\item The pushforward \reviewer[4][-0.5ex]{$\phi_*\mu^{\otimes2}$}{$(\phi\times\phi)_*(\mu^{\otimes2})$} coincides with \reviewer[3][2ex]{$\mu'^{\otimes2}$}{$(\mu')^{\otimes2}$} on rectangles, since
\[
(\phi_*\mu^{\otimes2})(A'\times B')=\mu^{\otimes2}\bigl(\phi^{-1}(A')\times\phi^{-1}(B')\bigr)=\mu(\phi^{-1}(A'))\,\mu(\phi^{-1}(B'))
\]
which by~(M4) equals \reviewer[3]{$\mu'(A')\,\mu'(B')=\mu'^{\otimes2}(A'\times B')$}{$(\mu')^{\otimes2}$}, using the rectangle rule.
\end{itemize}
\end{remark}

\begin{proposition}[Composition and identity]\label{prop:category}
Morphisms of structural models compose: if $\phi:\mathcal M\to\mathcal M'$ and $\psi:\mathcal M'\to\mathcal M''$ are morphisms (sharing \reviewer[1]{a common $\eta$}{a common value of $\eta$}), then $\psi\circ\phi:\mathcal M\to\mathcal M''$ is a morphism. 
Moreover, the identity map \reviewer[3]{$\mathrm{id}_X:\mathcal M\to\mathcal M$}{$\text{id}_\mathcal{M}$} is a morphism. 
Hence structural models with a common value of $\eta\in[0,1)$ (with $E_0$ allowed to vary as object data) together with their morphisms form a category, 
\reviewer[2]{which we denote by $\mathbf{Struct}_{\eta}$}{Redefinition}.
\end{proposition}

\begin{proof}
We check \reviewer[1]{the four morphism axioms}{the four axioms of morphisms} for $\psi\circ\phi$.
...
(M4) \reviewer[1]{Measure--preservation,}{Measure preservation:} for $B''\in\mathcal A''$:
\[
\mu((\psi\circ\phi)^{-1}(B''))=\mu(\phi^{-1}(\psi^{-1}(B'')))=\mu'(\psi^{-1}(B''))=\mu''(B'').
\]
The \reviewer[1]{product--measure}{product-measure} identity follows analogously. \reviewer[1][2ex]{Identity morphism}{The identity morphism}  is trivial.
\end{proof}

\subsection{Preservation of the coupling law}

Morphisms of structural models preserve not only \reviewer[1]{the listed data}{the constituent data} but also the coupling law derived from Axiom~III.
...
\item \reviewer[1]{\emph{(Source--side inequality)}}{Source-side} Under the hypothesis $(\phi\times\phi)^{-1}(G')\subseteq G$, for every $B'\in\mathcal A'$,
\[
...
\]
}{$(\mu')^{\otimes2}$; Replace the logical quantifier $\forall$ with plain text.}
\end{enumerate}
\end{proposition}
...
\]
\begin{structuralreview}[1]{Grammatical fragment.}
...
\end{structuralreview}
\emph{Part (ii).} Under the stronger hypotheses $(\phi\times\phi)^{-1}(G')=G$ and $\phi$ surjective, the inclusion in the displayed identity becomes \reviewer[1]{equality}{an equality}:
\[
...
Let $\mathcal M=(X,\allowbreak\mathcal A,\allowbreak\mu,\allowbreak\mu^{\otimes2},\allowbreak R,\allowbreak I,\allowbreak\Pi_R,\allowbreak G,\allowbreak E_0,\allowbreak\eta)$ be a structural model. The map $\Pi_R:X\to X$ is idempotent ($\Pi_R\circ\Pi_R=\Pi_R$) at the level of the underlying measurable-relation data. Restricting 
\reviewer[1]{codomain}{the codomain} to $R\in\mathcal A$, $\Pi_R$ factors as
\[
...
\begin{proof}
\reviewer[1]{Idempotence is Axiom~I.}{Idempotence is guaranteed by Axiom~I.} The factorization $\Pi_R=\iota_R\circ\rho_R$ is definitional: $\rho_R$ is simply $\Pi_R$ with codomain restricted to $R$, and $\iota_R:R\to X$ is the \reviewer[1]{set--theoretic}{set-theoretic} inclusion. The identity $\rho_R\circ\iota_R=\mathrm{id}_R$ follows from $\Pi_R|_R=\mathrm{id}_R$ (Axiom~I).
\end{proof}
...
\begin{proof}
\reviewer[1]{Idempotence is Axiom~II;}{Idempotence is guaranteed by Axiom~II.} Proposition~\ref{prop:elementary}(iii) \reviewer[2][2ex]{upgrades}{identifies} reflexive, symmetric, idempotent relations to equivalence relations.
\end{proof}

\begin{remark}[Monad--comonad perspective]\label{rem:monad-comonad}
Propositions~\ref{prop:retraction-idempotent} and~\ref{prop:G-idempotent} together place the two structural idempotents of a structural model in analogy with the two sides of an idempotent \reviewer[3]{monad--comonad}{monad-comonad} pair:
\begin{itemize}[nosep]
...
Let $\mathbf{Struct}^{\mathrm{id}}_{\eta}\subseteq\mathbf{Struct}_{\eta}$ be the full subcategory consisting of models $\mathcal M$ with $\Pi_R=\mathrm{id}_X$ \reviewer[2]{(so $R=X$, $I=\varnothing$)}{Narrative flow.}.
\end{definition}

\begin{proposition}[Restriction functor, under fiber hypotheses]\label{prop:restriction-functor}
Let $\mathbf{Struct}_{\eta}^{\mathrm{fib}}\subseteq\mathbf{Struct}_{\eta}$ denote the full subcategory whose objects are those structural models $\mathcal M$ satisfying the measurability hypothesis of Proposition~\ref{prop:fiber-annihilation} (namely $\{r\}\in\mathcal A$ and $F_r=\Pi_R^{-1}(\{r\})\in\mathcal A$ for every $r\in R$) together with one of the regularity hypotheses \emph{(R--fin)} or \emph{(R--ctbl)} of that proposition. 
\begin{structuralreview}[2]{Information Overload. Use a more concise summary that refers back to the original definition of a structural model. }
Then the assignment
\[
...
\emph{Axiom III(a) on $\mathcal M|_R$.} We must \reviewer[1]{show}{show that} $\mu(R)>0$. By Proposition~\ref{prop:fiber-annihilation}, \reviewer[1]{$\mu(X\setminus R)=0$}{we obtain that $\mu(X\setminus R)=0$}. Since $I\subseteq X\setminus R$ by construction, \reviewer[1][2ex]{$\mu(I)=0$}{it induces that $\mu(I)=0$}. Axiom~III(a) for $\mathcal M$ gives $\mu(R)+\mu(I)=E_0(\mathcal M)>0$, hence $\mu(R)=E_0(\mathcal M)>0$. We record this positive scalar as $E_0(\mathcal M|_R):=\mu(R)$.

\reviewer[1]{
\emph{Axiom III(b) on $\mathcal M|_R$.} Trivial, since $\Pi_{R|_R}=\mathrm{id}_R$.
}{Axiom III(b) on $\mathcal{M}|_R$ is immediate from $\Pi_R|_R = \mathrm{id}_R$.}
...
\]
Since $\mu(X\setminus R)=0$, \reviewer[1]{monotonicity}{the monotonicity} and the rectangle rule give
\[
...
\]
\reviewer[1]{and similarly}{Similarly, we obtain that} $\mu^{\otimes2}(S')=0$ for every measurable $S'\subseteq(X\setminus R)\times X$. Consequently,
\[
\mu^{\otimes2}((B\times X)\cap G)=\mu^{\otimes2}((B\times R)\cap G)=\mu^{\otimes2}((B\times R)\cap G|_R),
\]
where the last equality \reviewer[1]{uses}{holds for} $B\subseteq R$ so 
\reviewer[3][2ex]{$(B\times R)\cap G=(B\times R)\cap G\cap(R\times R)=(B\times R)\cap G|_R$.}{Recommend moving to a display environment.} For the \reviewer[1]{right--hand}{right-hand} side of~$(\ast)$, note that $\Pi_R^{-1}(B)\setminus B\subseteq X\setminus R$: indeed, if $x\in\Pi_R^{-1}(B)\cap R$ then $\Pi_R(x)=x\in B$ (Axiom~I), so $x\in B$; the contrapositive gives $\Pi_R^{-1}(B)\setminus B\subseteq X\setminus R$. Hence
\[
...
\emph{Functoriality.} If $\phi:\mathcal M\to\mathcal M'$ is a morphism in $\mathbf{Struct}_{\eta}^{\mathrm{fib}}$, then $\phi(R)\subseteq R'$ by Remark~\ref{rem:morphism-consequences}, so $\phi|_R:R\to R'$ is \reviewer[1]{well--defined}{well-defined}. Axioms~(M1)--(M4) for $\phi|_R$ follow by restriction from those of $\phi$. Composition and identity are preserved, so $\rho$ is a functor.
\end{proof}

\begin{remark}[The restriction functor as a retraction of categories]\label{rem:restriction-retraction}
The subcategory $\mathbf{Struct}^{\mathrm{id}}_{\eta}$ includes canonically into $\mathbf{Struct}_{\eta}^{\mathrm{fib}}$ via the inclusion $\iota$ that regards an \reviewer[3]{identity--retraction}{identity-retraction} object as already satisfying the fiber hypotheses trivially (with $F_r=\{r\}$ for every $r\in R=X$). The composition $\rho\circ\iota$ is the identity on $\mathbf{Struct}^{\mathrm{id}}_{\eta}$: restricting an \reviewer[3]{identity--retraction}{identity-retraction} model to $R=X$ returns the model unchanged. In this precise sense, $\rho$ is a retraction of $\mathbf{Struct}_{\eta}^{\mathrm{fib}}$ onto its subcategory $\mathbf{Struct}^{\mathrm{id}}_{\eta}$, mirroring at the categorical level the \reviewer[1]{object--level}{object-level} retraction $\Pi_R:X\to R$.
\end{remark}
\begin{structuralreview}[2]{Overly dense.}
\begin{remark}[Connection to the classification theorem]\label{rem:category-classification}
\sloppy
Within the full subcategory of $\mathbf{Struct}^{\mathrm{id}}_{\eta}$ whose objects satisfy the full hypotheses of Theorem~\ref{thm:pi-id-classification}, that theorem provides a \reviewer[3]{dichotomy}{Remove it.}: for every $G$--equivalence class $C_k$, either $\mu(C_k)=0$ or $\mu(C_k)=(1-\eta)^{-1}$. Equivalently, every positive--mass class satisfies $\mu(C_k)=(1-\eta)^{-1}$. On that subcategory, each object can be described combinatorially by a partition $(X,(C_k)_{k\in K})$ together with admissible mass data on the classes (namely the dichotomy $\mu(C_k)\in\{0,(1-\eta)^{-1}\}$) and the induced product charge on $X\times X$; morphisms preserve the partition structure and the measure/charge data in the sense of Definition~\ref{def:morphism}. The restriction functor $\rho$ of Proposition~\ref{prop:restriction-functor} extracts this combinatorial core from an arbitrary structural model under the fiber hypotheses, provided the core satisfies those further hypotheses.
\end{remark}
\end{structuralreview}
\subsection{Quotient factorization and structural reduction}\label{sec:quotient-factorization}

We now sharpen the restriction functor of Proposition~\ref{prop:restriction-functor} into a structural reduction theorem: under fiber measurability and regularity hypotheses, all \reviewer[3]{curvature--load}{curvature-load} information relevant to Axioms~I--III reduces to the \reviewer[3]{identity--retraction}{identity-retraction} core $\mathcal M|_R$, and the coupling law on $\mathcal M$ is equivalent to the coupling law on $\mathcal M|_R$. This extends the classification theorem (Theorem~\ref{thm:pi-id-classification}), which addresses the case $\Pi_R=\mathrm{id}_X$, to the general case.
...
\]
Since $\Pi_R$ is a function into $R$, the fibers $\{F_r\}_{r\in R}$ always partition $X$ \reviewer[1]{as a disjoint union}{into disjoint fibers} indexed by the image of $\Pi_R$; if moreover Axiom~I holds, then $\Pi_R|_R=\mathrm{id}_R$ so $r\in F_r$ for every $r\in R$, and $X=\bigsqcup_{r\in R}F_r$.
\end{definition}

The fiber decomposition is the \reviewer[1]{set--theoretic}{set-theoretic} quotient of $X$ under the equivalence relation $x\sim_\Pi y\iff\Pi_R(x)=\Pi_R(y)$; if Axiom~I holds, this quotient is canonically identified with $R$ via $r\leftrightarrow F_r$.

\begin{proposition}[Fiber--measure annihilation]\label{prop:fiber-annihilation}
\reviewer[1]{Assume}{Assume that} Axiom~I holds. Assume moreover that $\{r\}\in\mathcal A$ and $F_r\in\mathcal A$ for every $r\in R$, and that one of the following global regularity hypotheses holds:
\begin{itemize}[nosep]
\item[\emph{(R--fin)}] \reviewer[1]{$R$ is finite; or}{Remove "or". Recommend that $R$-fin and $R$-ctbl.}
\item[\emph{(R--ctbl)}] $R$ is countable and $\mu$ is \reviewer[3]{$\sigma$--additive}{$\sigma$-additive} on $\mathcal A$ (so $\mathcal A$ is assumed to be a $\sigma$--algebra).
\end{itemize}
...
\begin{proof}
\reviewer[2]{By Axiom~I, $\Pi_R|_R=\mathrm{id}_R$, so $\Pi_R(r)=r$ and $r\in F_r=\Pi_R^{-1}(\{r\})$ for every $r\in R$.}{Lack of description} Hence
\[
...
For the global claim $\mu(X\setminus R)=0$, \reviewer[1]{decompose}{We have a decomposition}
\[
X\setminus R=\bigsqcup_{r\in R}(F_r\setminus\{r\}).
\]
Under (R--fin) this is a finite disjoint union of null sets, and finite additivity gives $\mu(X\setminus R)=0$. Under (R--ctbl) the union is countable, and $\sigma$--additivity of $\mu$ applied to the countable disjoint union \reviewer[1]{gives again $\mu(X\setminus R)=0$}{gives $\mu(X\setminus R)=0$ again}.
\end{proof}

\begin{remark}[On the regularity hypothesis]\label{rem:regularity-hypothesis}
Without either hypothesis (R--fin) or (R--ctbl), the \reviewer[3]{fiber--wise}{fiber-wise} conclusion $\mu(F_r\setminus\{r\})=0$ still holds for each individual $r\in R$, but the global conclusion $\mu(X\setminus R)=0$ may fail, since finite additivity alone does not control infinitely many null sets. A finitely additive measure on \reviewer[1]{an uncountable $R$}{an uncountable set $R$} may assign positive total mass to an uncountable disjoint union of null sets. 
\reviewer[1]{The hypotheses (R--fin), (R--ctbl)}{The hypotheses (R-fin) and (R-ctbl)} are the natural setting for the structural reduction theorem below; extensions to the uncountable $\sigma$--additive case require additional assumptions (e.g., \reviewer[1]{separability of $\mathcal A$}{the separability of $\mathcal A$}) which we do not pursue.
\end{remark}

\begin{remark}[Relation to Proposition~\ref{prop:muI-zero}]\label{rem:fiber-vs-muI}
Proposition~\ref{prop:fiber-annihilation} strengthens Proposition~\ref{prop:muI-zero} (support concentration under \reviewer[1]{full partition}{a full partition}): the full partition hypothesis $X=R\sqcup I$ is replaced by the weaker measurability condition on fibers. The conclusion $\mu(X\setminus R)=0$ now holds for any structural model whose fibers are measurable, 
\reviewer[1]{regardless of whether $X=R\sqcup I$}{even in the absence of the full partition condition $X = R \sqcup I$.}.
\end{remark}
...
\item (\emph{\reviewer{curvature}{Curvature} load reduces to $R$}) If $\mathcal D$ satisfies Axiom~I and Axiom~III(b), then $\mu(X\setminus R)=0$, and for all $B\in\mathcal A$, writing $B_R:=B\cap R$,
\[
\mu^{\otimes2}\bigl((B\times X)\cap G\bigr)=\mu^{\otimes2}\bigl((B_R\times R)\cap (G|_R)\bigr).
\]
\item (\emph{\reviewer{coupling}{Coupling} laws are equivalent}) If $\mathcal D$ satisfies Axiom~I and Axiom~III(b), then the coupling law (Axiom~III(c)) holds on $\mathcal D$ if and only if it holds on $\mathcal D|_R$.
\item (\emph{\reviewer{admissibility}{Admissibility} reduces}) If $\mathcal D$ satisfies Axioms~I, II, III(a), and~III(b) as well as the full partition condition $X=R\sqcup I$, then $\mathcal D$ is an admissible structural model (i.e., an object of $\mathbf{Struct}_\eta$) if and only if $\mathcal D|_R$ is an admissible structural model (i.e., an object of $\mathbf{Struct}^{\mathrm{id}}_\eta$). In particular, whenever the restricted core $\mathcal D|_R$ also satisfies the full hypotheses of Theorem~\ref{thm:pi-id-classification}, the classification of admissible structural models reduces to the 
\reviewer[3]{identity--retraction}{identity-retraction} classification of Theorem~\ref{thm:pi-id-classification}. (The hypothesis ``Axiom~III(b)'' is not redundant: the datum \reviewer[3]{$\mathcal D_{\neg\mathrm{III}}$}{Notaion flaw} of Proposition~\ref{prop:indep-III} satisfies Axioms~I, II, III(a), III(c), fiber measurability, (R--fin), and the full partition $X=R\sqcup I$, yet fails Axiom~III(b); its restriction $\mathcal D_{\neg\mathrm{III}}|_R$ is admissible while $\mathcal D_{\neg\mathrm{III}}$ itself is not, so the equivalence fails without~III(b).)
\end{enumerate}
\end{theorem}
...
\]
The second term lies in $X\times(X\setminus R)$, hence \reviewer[1]{has}{it has} $\mu^{\otimes2}$--measure zero. The third term lies in $(X\setminus R)\times X$ (since $B\setminus R\subseteq X\setminus R$), hence also has $\mu^{\otimes2}$--measure zero. Therefore
\[
\mu^{\otimes2}((B\times X)\cap G)=\mu^{\otimes2}((B_R\times R)\cap G)=\mu^{\otimes2}((B_R\times R)\cap G|_R),
\]
where the last equality \reviewer[1]{uses}{is due to} $(B_R\times R)\cap G=(B_R\times R)\cap(G\cap(R\times R))=(B_R\times R)\cap G|_R$, valid \reviewer[1]{because}{because of} $B_R\subseteq R$.
...
\]
\reviewer[1]{Since $\mu(X\setminus R)=0$ we have $\mu(B)=\mu(B_R)$.}{Since $\mu(X\setminus R)=0$, we have $\mu(B)=\mu(B_R)$.} Within $\mathcal D|_R$, the retraction is $\mathrm{id}_R$, so $\Pi_R^{-1}(B_R)\cap R=B_R$. Substituting:
\[
...
\]
where $\{C_k\}_{k\in K}$ \reviewer{are}{is} the $G|_R$--equivalence classes of the identity--retraction core $\mathcal D|_R$, and $\mu(C_k)\in\{0,(1-\eta)^{-1}\}$ for each class, with $\mu(C_k)=(1-\eta)^{-1}$ on positive--mass classes.
\end{corollary}

\begin{proof}
\reviewer[1]{Combine}{This is immediate from combining} Theorem~\ref{thm:quotient-factorization}(i) with Theorem~\ref{thm:pi-id-classification}, applied to the admissible core $\mathcal D|_R$ under its full hypotheses.
\end{proof}
...
R := \{r_j : j\in J\},\qquad I := X\setminus R.
\]
One may view $R$ as the observable representative sector and $I$ as a hidden sector; we define $\Pi_R\colon X\to R$ by $\Pi_R(x)=r_j$ whenever $x\in C_j$.
