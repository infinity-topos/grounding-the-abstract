...
\begin{proof}
\reviewer[1][-2ex]{(i) follows from}{(i) Follows from} $\Pi_R(X)\subseteq R$ and $\Pi_R|_R=\mathrm{id}_R$ (Axiom~I). \reviewer{(ii) is a consequence}{(ii) Is a consequence} of the idempotence $\Pi_R\circ\Pi_R=\Pi_R$: for $n=1$ this is definitional, and for $n\ge 2$ we have $\Pi_R^n(x)=\Pi_R(\Pi_R^{n-1}(x))=\Pi_R(x)$ by induction. \reviewer{(iii) is immediate}{(iii) Is immediate} from Axiom~II as stated.
\end{proof}
