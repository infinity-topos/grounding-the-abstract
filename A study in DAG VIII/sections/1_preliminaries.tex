...
\begin{theorem}[Homological Whitehead Theorem with Twisted Coefficients]
  Let $f: X \to Y$ be a continuous map between connected CW complexes. Suppose that the following two conditions hold:
  \begin{enumerate}
    \item The map induces an isomorphism on the fundamental groups, i.e., $f_*: \pi_1(X) \xrightarrow{\cong} \pi_1(Y)$. Let $G = \pi_1(Y)$.
    \item For every local system of coefficients $\mathcal{L}$ on $Y$ (regarded as a $\mathbb{Z}[G]$-module), the induced homomorphism on twisted homology
    \[
      f_*: H_*(X; f^*\mathcal{L}) \xrightarrow{\cong} H_*(Y; \mathcal{L})
    \]
    is an isomorphism.
  \end{enumerate}
  Then $f$ is a weak homotopy equivalence; that is, $f_*: \pi_n(X) \to \pi_n(Y)$ is an isomorphism for all $n \ge 1$.
\end{theorem}
